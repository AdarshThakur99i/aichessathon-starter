...
# Driving a bare king to the edge. A king is mated at the edge, so once the losing side has no
# pawns and is down a rook or more, the evaluator has to supply the plan: push their king outward
# and walk ours up. Without it the engine cannot win king and rook against king, because every
# move looks alike until mate is inside the horizon and it shuffles into a threefold instead.
#
...
# alpha anyway. This buys depth rather than trading it away, which is the opposite of what the
# decisive-advantage exit does. Captures, promotions, checks and positions in check are never
# reduced, because those are the moves whose value a missing ply is most likely to hide.
LMR_MIN_DEPTH = 3
LMR_MIN_MOVES = 3
LMR_DEEP_MOVES = 6
...
def _attack_summary(board: chess.Board, color: chess.Color) -> tuple[int, int]:
    """Return the mask of squares this side attacks, and its pseudo-mobility.
...
def _hanging_pieces(board: chess.Board, color: chess.Color) -> int:
    defended, _ = _attack_summary(board, color)
    attacked, _ = _attack_summary(board, not color)
...
# Two more pieces of established chess knowledge, at their established values rather than fitted
# ones. A pair of bishops covers both colour complexes, which is worth more than the sum of the
# two pieces, especially once the position opens. A rook is worth more on a file its own pawns
# have vacated, and more still on one neither side blocks.
BISHOP_PAIR_BONUS = 0.35 * PAWN_UNITS
ROOK_OPEN_FILE_BONUS = 0.20 * PAWN_UNITS
ROOK_SEMI_OPEN_FILE_BONUS = 0.10 * PAWN_UNITS
...
def _build_tables() -> tuple[list[list[int]], list[list[int]], list[list[int]], list[list[int]]]:
    """One table per piece type per colour, with the black side mirrored top to bottom."""
    white_mg: list[list[int]] = [[0] * 64 for _ in range(7)]
    white_eg: list[list[int]] = [[0] * 64 for _ in range(7)]
    black_mg: list[list[int]] = [[0] * 64 for _ in range(7)]
    black_eg: list[list[int]] = [[0] * 64 for _ in range(7)]
    for piece_type, (middlegame, endgame) in _PST_SOURCE.items():
        for square in chess.SQUARES:
            white_mg[piece_type][square] = middlegame[square]
            white_eg[piece_type][square] = endgame[square]
            mirrored = chess.square_mirror(square)
            black_mg[piece_type][square] = middlegame[mirrored]
            black_eg[piece_type][square] = endgame[mirrored]
    return white_mg, white_eg, black_mg, black_eg


_PST_WHITE_MG, _PST_WHITE_EG, _PST_BLACK_MG, _PST_BLACK_EG = _build_tables()
...
def _piece_square_score(board: chess.Board) -> float:
    ...
    for piece_type in (chess.PAWN, chess.KNIGHT, chess.BISHOP, chess.ROOK, chess.QUEEN, chess.KING):
        table_mg = _PST_WHITE_MG[piece_type]
        table_eg = _PST_WHITE_EG[piece_type]
        for square in chess.scan_forward(board.pieces_mask(piece_type, chess.WHITE)):
            middlegame += table_mg[square]
            endgame += table_eg[square]
        table_mg = _PST_BLACK_MG[piece_type]
        table_eg = _PST_BLACK_EG[piece_type]
...
def _bishop_pair(board: chess.Board) -> float:
    """One for a white pair, minus one for a black pair."""
    white = chess.popcount(board.bishops & board.occupied_co[chess.WHITE]) >= 2
    black = chess.popcount(board.bishops & board.occupied_co[chess.BLACK]) >= 2
    return float(white) - float(black)


def _rook_files(board: chess.Board) -> float:
    """Rooks on files their own pawns have left, scored from the white side.

    A file with no pawn of ours is half open and worth a bonus; one with no pawn of either side
    is fully open and worth more, because nothing stops the rook from using it.
    """
    total = 0.0
    for color, sign in ((chess.WHITE, 1.0), (chess.BLACK, -1.0)):
        ...
        for square in chess.scan_forward(board.rooks & own):
            ...
            if enemy_pawns & file_mask:
                total += sign * ROOK_SEMI_OPEN_FILE_BONUS
            else:
                total += sign * ROOK_OPEN_FILE_BONUS
    return total
...
def _mating_drive(board: chess.Board) -> float:
    ...
    rook or more ahead against an opponent with no pawns, which keeps it out of the way of
    ordinary positions entirely.
    """
    if board.pawns:
    ...
    if balance >= MATING_DRIVE_MIN_EDGE and not black_pawns:
        ...
        strong, weak, sign = chess.BLACK, chess.WHITE, -1.0
    else:
        return 0.0
...
def _white_evaluation(board: chess.Board) -> float:
    ...
    return (
        ...
        + BISHOP_PAIR_BONUS * _bishop_pair(board)
        + _rook_files(board)
        + _mating_drive(board)
    )
...
def _storm_files(board: chess.Board) -> frozenset[int]:
    """The files worth pushing pawns down, once the opponent's king has committed to a wing.
    ...
    if king is None or board.has_castling_rights(opponent):
        return frozenset()
...
def _negamax(
    ...
    best_score = -math.inf
    best_move = moves[0]
    reducible = depth >= LMR_MIN_DEPTH and not board.is_check()
    _path[key] = 1
    try:
        for index, move in enumerate(moves):
            ...
            if (
                ...
                and _capture_victim(board, move) is None
            ):
                reduction = 1 if index < LMR_DEEP_MOVES else 2
            board.push(move)
            if reduction and board.is_check():
                reduction = 0
            if index == 0:
                ...
                score = -_negamax(
                    board, depth - 1 - reduction, -alpha - 1.0, -alpha, ply + 1, deadline
                )
                if reduction and score > alpha:
                    # The shallow search liked it, so the reduction was wrong here: verify.
