...
# Null move pruning. If handing the opponent a free move still leaves the position good enough to
# fail high, the real move will be at least as good, so the whole subtree can be skipped. The
# guards matter: never in check, never when a mate score is at stake, never twice in a row, and
# never with only king and pawns left, because that is where passing is genuinely the best move
# available and the assumption behind the trick breaks down.
...
def _has_pieces(board: chess.Board, color: chess.Color) -> bool:
    """True when this side has something other than king and pawns, so passing is not its best."""
    own = board.occupied_co[color]
...
def _negamax(
    ...
    if (
        ...
        and not (board.move_stack and not board.move_stack[-1])
    ):
        board.push(chess.Move.null())
        passed = -_negamax(
            board, depth - 1 - NULL_MOVE_REDUCTION, -beta, -beta + 1.0, ply + 1, deadline
        )
        board.pop()
        if passed >= beta:
            # Report the bound rather than the score: a mate found behind a pass is not real.
            return beta

    moves = _ordered_moves(board, tt_move, ply)
    if not moves:
        return -MATE_SCORE + ply if in_check else _draw_score(board)

    best_score = -math.inf
    best_move = moves[0]
    reducible = depth >= LMR_MIN_DEPTH and not in_check
    _path[key] = 1
    try:
